...
for approach in ['zero', 'few']:
    ...
    for dataset in datasets.keys():
        ...
        for task in datasets[dataset]["tasks"]:
            ...
            print('\nPrompting of {} on {} dataset, task {}: {}-shot classification'.format(model_id, dataset, task, approach))
            ...
            for i in df_test["text"]:
                ...
                prompt = tokenizer.apply_chat_template(messages, chat_template="DE", tokenize=True, add_generation_prompt=True, return_tensors="pt")
                ...
                answer=tokenizer.decode(prediction[0], skip_special_tokens=True).split("Assistant:")[-1].strip().replace("</s>", "")
